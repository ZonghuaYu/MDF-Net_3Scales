...
ngroups = (16, 8, 4)#(32, 16, 8) #list(reversed(chs))#[16, 8, 4]   #[32, 16, 8]
Homoaggre = nn.ModuleList([VectorAggregate(ngroups[i])      # VectorAggregate_vector
                           for i in range(stages)])
# 3D convolution regularization method
Regular0 = RegularNet_4Scales(ngroups[0])#RegularNet_3Scales(ngroups[0])
...
# model
model = core.CoreNet(Backbone, Depth_hypos, scale, Homoaggre, Regular, Regress, Refine=None)
